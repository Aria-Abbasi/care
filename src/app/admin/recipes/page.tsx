      ...
      {activeTab === "diet" ? (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="bg-white p-5 rounded-3xl border border-slate-200 shadow-sm">
            <h2 className="text-base font-black text-slate-900 mb-3 pb-2 border-b border-slate-100 flex items-center gap-2">
            ...
            <div className="space-y-2">
              {lunches.map((item) => (
                <div
                  key={item.id}
                  className="p-3 rounded-2xl bg-slate-50 border border-slate-100 text-xs font-bold text-slate-800"
                >
                  {item.title}
                </div>
              ))}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {smoothies.map((sm) => (
            <div
              key={sm.id}
              className="p-5 rounded-3xl bg-white border border-slate-200 shadow-sm hover:border-slate-300 transition flex flex-col justify-between"
            >
              <div>
                <h2 className="text-sm font-black text-emerald-900 flex items-center gap-2 mb-2">
                  <Coffee className="w-4 h-4 text-emerald-600" />
                  <span>{sm.title}</span>
                </h2>
                <p className="text-xs text-slate-600 whitespace-pre-line leading-relaxed bg-emerald-50/40 p-3 rounded-2xl border border-emerald-100">
                  {sm.ingredients}
                </p>
              </div>
            </div>
        ...
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="w-full max-w-md bg-white rounded-3xl shadow-2xl p-6 border border-slate-100">
            <div className="flex items-center justify-between pb-3 border-b border-slate-100 mb-4">
              <h2 className="text-base font-black text-slate-900">افزودن دستور جدید</h2>
            ...
            <div className="space-y-3">
              <div>
                <label className="block text-xs font-bold text-slate-700 mb-1">نوع غذا / نوشیدنی</label>
                <select
                  value={formType}
              ...
              <div>
                <label className="block text-xs font-bold text-slate-700 mb-1">عنوان غذا یا اسموتی</label>
                <input
                  type="text"
                  value={formTitle}
              ...
              {formType === "SMOOTHIE" && (
                <div>
                  <label className="block text-xs font-bold text-slate-700 mb-1">ترکیبات و مواد لازم</label>
                  <textarea
                    rows={3}
